...
# Cho phép nhận diện cả 2 định dạng file Excel cũ và mới
    ...
    if st.button("Bắt đầu xử lý và tạo Tem PDF", type="primary"):
        with st.spinner("Hệ thống đang xử lý dữ liệu, vui lòng đợi..."):
            ...
            if path_pdf_result and tong_so_tem > 0:
                ...
                # Cắt đuôi tên file Excel cũ để ghép tên file PDF cho đồng bộ khi lưu máy
                ten_file_base_name = os.path.splitext(uploaded_file.name)[0]
                
                st.download_button(
                    label="📥 Nhấn vào đây để tải file PDF Tem về máy",
                    data=pdf_bytes,
                    file_name=f"{ten_file_base_name}.pdf",
